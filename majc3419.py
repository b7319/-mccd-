...
STRATEGIES = {'cluster': '三均线密集', 'cross': '双均线交叉'}

# 可由侧栏覆盖的运行配置
CONFIG = {
    'density_threshold_pct': 0.3,  # 百分比（0.3 表示 0.3%）
...
def detect_cluster_signals(data, symbol, timeframe):
    ...
    if data is None or len(data['closes']) < max(CONFIG['cluster_check_window'], CONFIG['cluster_recent_bars']):
        return res
    pct = CONFIG['density_threshold_pct']
...
def detect_cross_signals(data):
    ...
    short_term_indices = np.arange(len(closes))[-3:]
    ...
    for idx in recent_indices:
        try:
            if golden_cross_170_453[idx]:
                ...
                if current_price > cross_price and np.sum(golden_cross_170_453[-513:]) == 1:
                    has_match = any(d == '多头' and st_t >= cross_time and st_p >= cross_price for d, st_t, st_p in
                                    short_term_signals)
                    if has_match:
                ...
                cross_time = datetime.fromtimestamp(int(timestamps[idx]) / 1000, tz=beijing_tz);
                cross_price = closes[idx]
                if current_price > cross_price and np.sum(golden_cross_34_170[-513:]) == 1:
                    has_match = any(d == '多头' and st_t >= cross_time and st_p >= cross_price for d, st_t, st_p in
                                    short_term_signals)
                    if has_match:
                        valid_signals.append(('多头', signal_type, cross_time, cross_price))
                ...
                cross_time = datetime.fromtimestamp(int(timestamps[idx]) / 1000, tz=beijing_tz);
                cross_price = closes[idx]
                if current_price > cross_price and np.sum(golden_cross_34_453[-513:]) == 1:
                    has_match = any(d == '多头' and st_t >= cross_time and st_p >= cross_price for d, st_t, st_p in
                                    short_term_signals)
                    if has_match:
                        valid_signals.append(('多头', signal_type, cross_time, cross_price))
                ...
                cross_time = datetime.fromtimestamp(int(timestamps[idx]) / 1000, tz=beijing_tz);
                cross_price = closes[idx]
                if current_price < cross_price and np.sum(death_cross_170_453[-513:]) == 1:
                    has_match = any(d == '空头' and st_t >= cross_time and st_p <= cross_price for d, st_t, st_p in
                                    short_term_signals)
                    if has_match:
                ...
                cross_time = datetime.fromtimestamp(int(timestamps[idx]) / 1000, tz=beijing_tz);
                cross_price = closes[idx]
                if current_price < cross_price and np.sum(death_cross_34_170[-513:]) == 1:
                    has_match = any(d == '空头' and st_t >= cross_time and st_p <= cross_price for d, st_t, st_p in
                                    short_term_signals)
                    if has_match:
                        valid_signals.append(('空头', signal_type, cross_time, cross_price))
                ...
                cross_time = datetime.fromtimestamp(int(timestamps[idx]) / 1000, tz=beijing_tz);
                cross_price = closes[idx]
                if current_price < cross_price and np.sum(death_cross_34_453[-513:]) == 1:
                    has_match = any(d == '空头' and st_t >= cross_time and st_p <= cross_price for d, st_t, st_p in
                                    short_term_signals)
                    if has_match:
                        valid_signals.append(('空头', signal_type, cross_time, cross_price))
...
def process_symbol_timeframe(exchange, symbol, timeframe, failed_symbols):
    ...
    try:
        ...
        cross_list, current_price = detect_cross_signals(data)
...
# ========== 入口：侧栏参数 ==========
def main():
    st.title('一体化监控（v2） - 三均线密集 & 双均线交叉（1m/5m/30m）')
    st.sidebar.header('运行参数（可调整）')
    CONFIG['density_threshold_pct'] = st.sidebar.number_input('密集阈值 (%)', min_value=0.01, max_value=5.0, value=0.3,
                                                              step=0.01)
    CONFIG['cluster_recent_bars'] = st.sidebar.number_input('密集判定最近K线数 (X)', min_value=3, max_value=100,
                                                            value=13, step=1)
    CONFIG['cluster_check_window'] = st.sidebar.number_input('密集检查窗口 (Y)', min_value=10, max_value=200, value=86,
                                                             step=1)
    CONFIG['cross_cooldown_multiplier'] = st.sidebar.number_input('双均线交叉冷却倍数 (interval * X)', min_value=1,
                                                                  max_value=20, value=5, step=1)
    CONFIG['fetch_limit'] = st.sidebar.number_input('拉取K线数量 (fetch limit)', min_value=600, max_value=2000,
                                                    value=968, step=1)
