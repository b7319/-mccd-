...
TIMEFRAMES_KEYS = list(TIMEFRAMES.keys())
...
if 'result_containers' not in st.session_state:
    st.session_state.result_containers = {tf: {'container': None, 'placeholder': None} for tf in TIMEFRAMES_KEYS}
...
def get_valid_symbols():
    try:
        ...
        try:
            ...
            logging.warning(f"载入市场失败: {e}")
            markets = exchange.load_markets()

        try:
            tickers = exchange.fetch_tickers(params={'type': 'swap'})
        except Exception:
            # 某些交易所或 ccxt 版本不支持 params，尝试不传参
        ...
        for symbol, ticker in tickers.items():
            ...
            try:
                if (market.get('type') == 'swap' and
                    ...
                    logging.debug(f"有效交易对: {symbol}, 成交量: {ticker.get('quoteVolume')}")
            except Exception:
                continue
...
def get_cached_ohlcv(symbol, timeframe, failed_symbols):
    ...
    for attempt in range(3):
        try:
            ...
            if data and count >= 500:
                ohlcv_cache[cache_key] = (data, now)
                return data
            else:
                logging.warning(f"数据不足: {symbol}, {timeframe}, 获取到 {count} 条")
                return None
...
def check_ma_cluster(ma34, ma170, ma453, pct_threshold=0.003):
    ...
    try:
        # 获取当前均线值
        current_ma34 = float(ma34[-1])
        current_ma170 = float(ma170[-1])
        current_ma453 = float(ma453[-1])
...
def detect_signals(data, timeframe):
    ...
    # 获取当前均线值
    current_ma34 = float(data['ma34'][-1])
    current_ma170 = float(data['ma170'][-1])
    current_ma453 = float(data['ma453'][-1])
...
def process_symbol_batch(symbol, failed_symbols):
    signals = []
    for timeframe in TIMEFRAMES_KEYS:
        try:
...
def monitor_symbols():
    """监控交易对"""
    tabs = st.tabs([f"{tf.upper()} 周期" for tf in TIMEFRAMES_KEYS])
    for idx, tf in enumerate(TIMEFRAMES_KEYS):
        with tabs[idx]:
        ...
        while True:
            ...
            for i, future in enumerate(as_completed(futures)):
                try:
                    ...
                    logging.debug(f"任务执行异常: {e}")
                    continue

                progress = (i + 1) / len(futures) if len(futures) > 0 else 1
